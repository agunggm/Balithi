<div id="page-wrapper">
  <div class="row">
    <div class="col-lg-12">
      <h1 class="page-header">Data Permintaan Materi</h1>
    </div>
    <!-- /.col-lg-12 -->
  </div>
  <!-- /.row -->
              ...
              foreach ($permintaan as $key => $value) {
                ...
                <tr>
                  ...
                  <td><button class="btn btn-info" data-toggle='modal' data-target="#rincian<?php echo $i;?>">
                    <?php if ($value['status_minta']==0) {
                      echo "Verifikasi";
                    }
                    else {
                      echo "Rincian";
                    } ?></button></td>
                    <td><a href="<?php echo base_url('permintaan/hapus/'.$value['id_permintaan']);?>">Hapus </td>
                    </tr>
                    <?php
                    $i++;
                  }
    ...
    <div class="modal fade" id="rincian<?php echo $i;?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="myModalLabel">Rincian Permintaan</h4>
          </div>
          <div class="modal-body">
            ...
            <div class="modal-footer">
              <?php if ($value['status_minta']==0) {?>
                <form action="<?php echo base_url(). 'permintaan/ubah_status'; ?>" method="post">
                  <input type="hidden" name="id" value="<?php echo $value['id_permintaan']?>">
